...
export const timelineNodes: TimelineNode[] = [
  ...
  },
  {
    ...
    detail: {
      ...
      star: {
        situation: '真实旅行规划不是一次性文本生成：用户意图模糊，景点营业时间、交通距离和偏好会相互影响；纯 SFT 容易学会路线格式，却不直接优化“走不走得通”，一步选错就产生 compounding error。',
        task: '我负责把路线规划建模成 next-POI 序贯决策，让模型在用户意图、已选景点和候选 POI 状态下选择下一站，同时把时间窗、移动时间和偏好匹配转成可验证优化目标。',
        action: '我构建语义/行为/地理三路召回和 4 类对抗干扰集，用 OR-Tools VRPTW verifier 提供硬约束 reward，再以 Qwen3-8B + LoRA 做 SFT → DPO → GRPO，并监控 KL、格式合法率、路线长度和 reward hacking。',
        result: '端到端路线可行率达到 92%，约束违规率从 40% 降到 8%，GRPO 单模块可行性约 88%；系统在同名 POI、伪捷径和跨文档时间窗干扰下更稳定。',
      },
    },
  },
  {
    ...
    hideTypeBadge: true,
    status: 'EvoMap 黑客松第一',
    title: '联想实习 · KTClaw · 企业数字员工系统',
    org: '联想 / EvoMap Hackathon',
    oneLiner: '面向企业知识与办公流程的自进化数字员工系统，用多入口接入、权限边界、工具调用和分身记忆支撑知识问答、售前客服与自动化办公。',
    metrics: [
      { before: '40k+', after: '2,298', label: '上下文 tokens' },
      { after: '-95%', label: '上下文压缩' },
      { after: '-90%', label: '推理成本' },
    ],
    keywords: ['企业数字员工', 'Multi-Agent', 'Memory', 'RAG-to-Skill'],
    detail: {
      background: 'Claw 面向售前、客服、HR、IT 支持和业务团队的高频知识与办公流程，不是简单的一问一答；它需要连接企业入口、知识库、工单系统和本地工具，同时处理组织权限、上下文污染和流程复用。',
      role: '核心算法与系统实现',
      approach: [
        '抽象 IM、Web 管理台、知识库、工单系统和本地工具等入口消息，将主 Agent 负责拆解、路由和汇总，专家 Agent 处理检索、工具执行、文档生成与核验。',
        '构建 index、fact、procedure、archive 四层记忆，并通过 metadata、semantic、procedure relation 多路召回动态组装上下文。',
        '通过权限过滤、失败回放、trace_id、Skill 修订和评测归因，把工具异常、检索污染、格式错误和成功/失败轨迹纳入迭代闭环。',
      ],
      tradeoffs: '多 Agent 能降低上下文串扰，但会带来调度和观测成本；实现上优先保留任务边界、权限过滤和关键链路 trace。',
      star: {
        situation: '企业数字员工要处理知识问答、售前客服、资料生成和跨系统自动化办公，但企业场景天然有权限边界、多入口消息、工具 observation 和历史记忆混杂的问题；如果全塞进 40k+ tokens，上下文污染会让模型把异常工具结果或中间草稿当最终证据。',
        task: '我负责在已有 OpenClaw Gateway 和多模型接入基础上，把系统从“接模型的聊天壳”推进到可执行企业流程的 Agent 底座：既要隔离任务阶段，又要让事实、流程和历史按权限与相关性进入动态上下文。',
        action: '我保留主 Agent 编排与专家 Agent 执行分工，构建 index/fact/procedure/archive 四层记忆，用 metadata、semantic、procedure relation 三路召回组装上下文，并通过 trace_id、失败回放和 Skill 修订把成功/失败轨迹沉淀成可复用 Skill。',
        result: 'KTClaw 获得 EvoMap 黑客松第一；代表性长任务从 40k+ tokens 压到约 2,298 tokens，压缩约 95%，内部典型任务错误率从 15% 降到 3%，推理成本降低约 90%。',
      },
    },
    links: [
  ...
  },
  {
    ...
    hideTypeBadge: true,
    status: '京东上架',
    title: '联想实习 · TinyClaw · 端侧 / Edge 数字员工',
    org: '联想 / 端侧项目',
    oneLiner: 'Claw 的端侧 / Edge 形态，面向私有数据、低延迟、本地工具和弱网场景，把高频流程沉淀为 SOP、Skill 与可执行脚本。',
    metrics: [
      { after: '3 层', label: '程序性记忆' },
      { after: '7k-10k', label: '季度预期销量' },
      { after: '闭环', label: '轨迹到 Skill' },
    ],
    keywords: ['端侧 / Edge', 'Procedural Memory', 'RAG-to-Skill', '云边协同'],
    detail: {
      background: 'TinyClaw 不是把云端 Agent 简单塞进小盒子，而是面向工厂、政企内网、会议室、客服中心等需要本地知识接入和本地工具执行的购买场景；端侧设备必须在私有数据、低延迟、合规和弱网环境下稳定复用流程。',
      role: '算法与工程实现',
      approach: [
        '记录 query、检索摘要、工具调用、输出和反馈，从成功/失败轨迹抽取前置条件、步骤、参数槽位和验证点。',
        '设计 SOP、Skill、可执行脚本三层程序性记忆，让经验按成熟度从人可读流程升级为结构化技能或受限执行器。',
        '用轻量向量检索和任务类型过滤召回候选 Skill，命中失败时回退完整 RAG 并进入修订队列，同时为云边协同保留脱敏上传和通用 Skill 下发接口。',
      ],
      tradeoffs: '端侧自进化不能让 Skill 库无限膨胀；需要按成功率、最近验证、工具版本和安全风险做降权、归档或回滚。',
      star: {
        situation: '端侧 / Edge 数字员工面对的是私有数据、本地工具、低延迟和弱网约束，不能每次都依赖云端长上下文；高频企业流程更适合沉淀成可复用做法，而不是反复完整 RAG。',
        task: '我负责把“检索到答案”升级成“学会做法”：让端侧系统从成功/失败轨迹中抽取前置条件、参数槽位、验证点和回退策略，并把云端 Claw 的记忆分层思想压缩成轻量本地流程。',
        action: '我设计 SOP、Skill、可执行脚本三层程序性记忆，打通轨迹记录、经验抽取、相似 Skill 召回、Skill 修订和复用验证；命中失败时回退完整 RAG，并为云边协同预留脱敏 Skill 上传和通用 Skill 下发路径。',
        result: 'TinyClaw 完成京东上架准备，形成面向端侧的 RAG-to-Skill 自进化方案；季度预期销量约 7k-10k 台，并明确用命中率、复用成功率、P50/P95 延迟和失败回退率评估后续迭代。',
      },
    },
  },
  {
  ...
  },
  {
    ...
    detail: {
      ...
      star: {
        situation: 'Agent 方向论文、工具、框架和工程案例增长很快，资料分散在仓库、博客和社区讨论中；读者真正缺的不是更多链接，而是能区分原理、工程范式和实践问题模式的结构化入口。',
        task: '我的目标是把分散资料整理成可检索、可持续维护的 Agent 知识图谱，让项目既能覆盖学习路径，也能沉淀真实工程实践中的常见问题和解决思路。',
        action: '我基于 OpenClaw 搭建爬取、清洗、合并流水线，用 LLM 从 30k+ 资料与实践样本中抽取高频知识点，再按原理层、工程层、实践层组织主题索引和链接校验。',
        result: 'AgentGuide 获得 5.4k+ GitHub Stars，进入 Multi-Agent topic Top 20；项目把资料收集转成持续维护的知识图谱，体现了开源影响力和工程自动化能力。',
      },
    },
