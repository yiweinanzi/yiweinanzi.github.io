...
export const timelineNodes: TimelineNode[] = [
  ...
  },
  {
    ...
    detail: {
      ...
      star: {
        situation: '企业长任务的核心风险是上下文污染：检索片段、工具 observation、用户澄清、文档草稿和历史记忆混在 40k+ tokens 里，模型容易把异常工具结果或中间草稿当最终证据。',
        task: '我负责把已有多入口和 OpenClaw Gateway 能力推进到企业 Agent 底座：既要隔离任务阶段，又要让事实、流程和历史能按权限与相关性进入动态上下文。',
        action: '我实现主 Agent 编排与专家 Agent 执行分工，构建 index/fact/procedure/archive 四层记忆，用 metadata、semantic、procedure relation 三路召回组装上下文，并用 trace_id 记录路由、检索、工具、模型和错误以支持失败回放。',
  ...
  },
  {
    ...
    detail: {
      ...
      star: {
        situation: '端侧 Agent 受算力、内存、延迟和隐私约束，不能每次都把任务交给云端长上下文；高频企业流程更适合沉淀成可复用做法，而不是反复完整 RAG。',
        task: '我负责把“检索到答案”升级成“学会做法”：让端侧系统从成功/失败轨迹中抽取前置条件、参数槽位、验证点和回退策略，并控制 Skill 库不要无限膨胀。',
        action: '我设计 SOP、Skill、可执行脚本三层程序性记忆，打通轨迹记录、经验抽取、相似 Skill 召回、Skill 修订和复用验证；命中失败时做失败回退到完整 RAG，并把新轨迹进入修订队列。',
