...
export const timelineNodes: TimelineNode[] = [
  ...
  },
  {
    ...
    detail: {
      ...
      star: {
        situation: '旅行规划 Agent 在真实场景中会遇到地点重名、营业时间冲突、路线不可达和用户偏好变化，纯文本生成很难稳定保证可行性。',
        task: '我的任务是把路线质量变成可验证奖励，并让 Agent 在复杂检索证据和工具调用中学会纠错与优化。',
        action: '我参与构建三路检索和对抗干扰集，用 OR-Tools VRPTW 设计可验证奖励，并将 GRPO 引入路线规划训练闭环。',
        result: '路线可行率从 60% 提升到 92%，约束违规率从 40% 降到 8%，同名地点和伪捷径等干扰下的纠错能力显著提升。',
      },
    },
  },
  {
  ...
  },
  {
    ...
    detail: {
      ...
      star: {
        situation: '黑客松要求在短周期内证明企业 Agent 的真实价值，而原始任务上下文常超过 40k tokens，推理成本和失败率都偏高。',
        task: '我需要设计一套能压缩上下文、复用流程经验、并支持多入口演示的 Agent 系统架构。',
        action: '我实现多 Agent 编排与四层记忆，使用多路检索挑选最相关事实和流程，并把失败轨迹沉淀为可修订 Skill。',
        result: '系统获得 EvoMap 黑客松第一，上下文从 40k+ 压到 2,298 tokens，压缩约 95%，推理成本降低约 90%。',
      },
    },
    links: [
  ...
  },
  {
    ...
    detail: {
      ...
      star: {
        situation: '端侧设备上的 Agent 需要低延迟、低成本、可持续改进，但云端大模型式的长上下文和频繁重规划不适合产品化。',
        task: '我的任务是把 RAG 检索进一步变成可复用的 Skill 记忆，让系统能从历史轨迹中自动提炼流程经验。',
        action: '我搭建三层程序性记忆，并实现从轨迹记录到经验抽取、相似召回、Skill 修订、复用验证的闭环。',
        result: 'TinyClaw 完成京东上架准备，形成面向端侧产品的自进化 Agent 方案，季度预期销量约 7k-10k 台。',
      },
    },
  },
  {
